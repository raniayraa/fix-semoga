#!/bin/bash
# Kernel experiment sweep: port count 1–10 × traffic directions (41, 15, 15_41)
#
# Usage:
#   bash run_kernel_sweep.sh [OPTIONS]
#
# Options:
...
LOG_FILE="/tmp/kernel_sweep_${SWEEP_TS}.log"
...
if not rows:
    print("  No Kernel sweep results with sweep_meta.json found yet.")
    sys.exit(0)
...
for n_ports in $(seq "$MIN_PORTS" "$MAX_PORTS"); do
  ...
  for direction in "${DIRECTIONS[@]}"; do
    for rep in $(seq 1 "$REPS"); do
      (( run++ )) || true
      run_label="Kernel | ports=${n_ports} (${port_label}) | dir=${direction} | rep=${rep}/${REPS}"
      ...
      if [[ $SKIP_SETUP -eq 0 ]]; then
        ...
        if [[ $setup_ok -eq 0 ]]; then
          log "  SKIP: setup playbook failed → ${run_label}"
      ...
      # ── [4] Kernel forwarder ─────────────────────────────────────────────────
      log "  [4/5] Configure Kernel forwarder ..."
      if ! run_playbook "$KERNEL_PLAYBOOK"; then
        log "  SKIP: Kernel playbook failed → ${run_label}"
        failed+=("$run_label"); continue
      fi
      ...
      if [[ $DRY_RUN -eq 1 ]]; then
        ...
        log "    [dry-run] result: Kernel_${port_label}_Port_No_Block_${direction}${rep_suffix}"
        continue
      fi
      ...
      if [[ -n "$new_dir" ]]; then
        rep_suffix=$([[ $REPS -gt 1 ]] && echo "_rep${rep}" || echo "")
        target_name="Kernel_${port_label}_Port_No_Block_${direction}${rep_suffix}"
