#!/bin/bash
# XDP experiment sweep: port count 1–10 × traffic directions (41, 15, 15_41)
#
# Usage:
#   bash run_xdp_sweep.sh [OPTIONS]
#
# Options:
...
LOG_FILE="/tmp/xdp_sweep_${SWEEP_TS}.log"
...
preflight() {
  ...
  if [[ -n "$q" ]]; then
    ...
    if (( q < 4 )); then
      ...
      log "  Fix: ensure start_turbo.sh sets ethtool -L combined <N> successfully."
    fi
  fi
}
...
if not rows:
    print("  No XDP sweep results with sweep_meta.json found yet.")
    sys.exit(0)
...
for n_ports in $(seq "$MIN_PORTS" "$MAX_PORTS"); do
  ...
  for direction in "${DIRECTIONS[@]}"; do
    for rep in $(seq 1 "$REPS"); do
      ...
      # ── [4] XDP forwarder ────────────────────────────────────────────────────
      log "  [4/5] Configure XDP forwarder ..."
      if ! run_playbook "$XDP_PLAYBOOK"; then
        log "  SKIP: XDP playbook failed → ${run_label}"
        failed+=("$run_label"); continue
      fi
      ...
      if [[ $DRY_RUN -eq 1 ]]; then
        ...
        log "    [dry-run] result: XDP_${port_label}_Port_No_Block_${direction}${rep_suffix}"
        continue
      fi
      ...
      if [[ -n "$new_dir" ]]; then
        rep_suffix=$([[ $REPS -gt 1 ]] && echo "_rep${rep}" || echo "")
        target_name="XDP_${port_label}_Port_No_Block_${direction}${rep_suffix}"
