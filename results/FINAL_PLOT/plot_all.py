...
def _detect_variant(folder_name):
    canonical = re.sub(r"_rep\d+$", "", folder_name)
    if re.search(r"_15_41$", canonical):
        return "Multi", "Multi Traffic"
    if re.search(r"_41$", canonical):
        return "Single_41", "Single Traffic"
    if re.search(r"_15$", canonical):
        return "Single_15", "Single Traffic"
    raise RuntimeError(
        f"Cannot detect traffic variant from folder name: {folder_name!r}\n"
        "Expected suffix '_15_41', '_41', or '_15' (optionally followed by '_repN')."
    )
...
for folder in data_folders:
    ...
    tech_match = re.match(r"(Kernel|VPP|XDP)_(\d+)-(\d+)_Port", folder_name)
    if not tech_match:
        print(f"Skipping {folder_name}: cannot parse technology/port count.")
        continue

    technology  = tech_match.group(1)
    port_start  = int(tech_match.group(2))
    port_end    = int(tech_match.group(3))
    n_port      = port_end - port_start + 1

    rep_match   = re.search(r"_rep(\d+)$", folder_name)
    rep_num     = int(rep_match.group(1)) if rep_match else 1

    title_line2 = f"{technology} - {traffic_label} - {n_port} Port (Rep {rep_num})"
    ...
    plt.close(fig)
    print(f"[plot]    result/plot_{folder_name}.png  [{technology} | {traffic_label} | {n_port} port | rep {rep_num}]")

    # accumulate data for summary plots
    base_key = (technology, variant_key)
    traffic_labels[base_key] = traffic_label
    rep_key = (technology, variant_key, rep_num)
    for rx_label, csv_file, metric, port, _y_max in RX_SPECS[variant_key]:
        try:
            ...
            summary_data[rep_key][rx_label].append((n_port, max_val))
        except Exception as exc:
            print(f"  Warning: {folder_name} [{rx_label}]: {exc}")

# ── pass 2: summary port-scaling plots (per-rep + averaged) ───────────────────
print()

# Group rep keys by (technology, variant_key)
base_keys = {}
for tech, variant, rep in summary_data:
    base_keys.setdefault((tech, variant), []).append(rep)

for (technology, variant_key), rep_nums in sorted(base_keys.items()):
    rep_nums_sorted = sorted(rep_nums)
    traffic_label   = traffic_labels[(technology, variant_key)]
    rx_spec_list    = RX_SPECS[variant_key]
    n_rx            = len(rx_spec_list)

    # per-rep summary plots
    for rep_num in rep_nums_sorted:
        rep_key = (technology, variant_key, rep_num)
        fig, axes = plt.subplots(1, n_rx, figsize=(14 * n_rx, 7))
        if n_rx == 1:
            axes = [axes]
        for ax, (rx_label, _csv, _metric, _port, y_max) in zip(axes, rx_spec_list):
            points = sorted(summary_data[rep_key].get(rx_label, []))
            draw_summary_ax(ax, rx_label, points, technology, traffic_label,
                            y_max=y_max)
        plt.tight_layout(pad=4.0)
        stem = os.path.join(result_dir, f"summary_{technology}_{variant_key}_rep{rep_num}")
        plt.savefig(f"{stem}.png", dpi=150, bbox_inches="tight")
        plt.savefig(f"{stem}.svg",           bbox_inches="tight")
        plt.close(fig)
        print(f"[summary] result/summary_{technology}_{variant_key}_rep{rep_num}.png  [{technology} | {traffic_label} | rep {rep_num}]")

    # averaged summary plot
    fig, axes = plt.subplots(1, n_rx, figsize=(14 * n_rx, 7))
    if n_rx == 1:
        axes = [axes]
    for ax, (rx_label, _csv, _metric, _port, y_max) in zip(axes, rx_spec_list):
        ...
        for rep_num in rep_nums_sorted:
            for port, mpps in summary_data[(technology, variant_key, rep_num)].get(rx_label, []):
                port_vals[port].append(mpps)
        avg_points = sorted((port, np.mean(vals)) for port, vals in port_vals.items())
        draw_summary_ax(ax, rx_label, avg_points, technology, traffic_label,
                        y_max=y_max)
    plt.tight_layout(pad=4.0)
    stem = os.path.join(result_dir, f"summary_{technology}_{variant_key}_avg")
    plt.savefig(f"{stem}.png", dpi=150, bbox_inches="tight")
    plt.savefig(f"{stem}.svg",           bbox_inches="tight")
    plt.close(fig)
    print(f"[summary] result/summary_{technology}_{variant_key}_avg.png  [{technology} | {traffic_label} | avg over {len(rep_nums_sorted)} reps]")
